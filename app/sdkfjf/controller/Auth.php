...
use app\sdkfjf\model\AuthGroup;
use app\sdkfjf\model\authRule;
use think\Validate;
class Auth extends Common
{
    ...
    public function adminAdd(){
        if(request()->isPost()){
            ...
            if (Admin::create($data)) {
                return ['code'=>1,'msg'=>'管理员添加成功!','url'=>url('sdkfjf/Auth/adminList')];
            } else {
    ...
    public function adminState(){
        ...
        if (empty($id)){
            $result['status'] = 0;
            $result['info'] = '用户ID不存在!';
            $result['url'] = url('admin/Auth/adminList');
            exit;
        }
    ...
    public function adminEdit(){
        if(request()->isPost()){
            ...
            Admin::update($data);
            return $result = ['code'=>1,'msg'=>'管理员修改成功!','url'=>url('sdkfjf/Auth/adminList')];
        }else{
    ...
    public function groupAdd(){
        if(request()->isPost()){
            ...
            $result['msg'] = '用户组添加成功!';
            $result['url'] = url('sdkfjf/Auth/adminGroup');
            $result['code'] = 1;
            return $result;
        }else{
    ...
    public function groupEdit(){
        if(request()->isPost()) {
            ...
            $result = ['code'=>1,'msg'=>'用户组修改成功!','url'=>url('sdkfjf/Auth/adminGroup')];
            return $result;
        }else{
            $id = input('id');
    ...
    public function groupSetaccess(){
        ...
        if(AuthGroup::update($data)){
            return array('msg'=>'权限配置成功!','url'=>url('sdkfjf/Auth/adminGroup'),'code'=>1);
        }else{
    ...
    public function ruleAdd(){
        if(request()->isPost()){
            ...
            return $result = ['code'=>1,'msg'=>'权限添加成功!','url'=>url('sdkfjf/Auth/adminRule')];
        }else{
            $nav = new Leftnav();
            $arr = cache('authRuleList');
            if(!$arr){
    ...
    public function ruleOrder(){
        ...
        if($auth_rule->update($data)!==false){
            cache('authRuleList', NULL);
            cache('authRule', NULL);
            return $result = ['code'=>1,'msg'=>'排序更新成功!','url'=>url('sdkfjf/Auth/adminRule')];
        }else{
    ...
    public function ruleEdit(){
        if(request()->isPost()) {
            ...
            if(authRule::update($datas)) {
                cache('authRule', NULL);
                cache('authRuleList', NULL);
                return json(['code' => 1, 'msg' => '保存成功!', 'url' => url('sdkfjf/Auth/adminRule')]);
            } else {
